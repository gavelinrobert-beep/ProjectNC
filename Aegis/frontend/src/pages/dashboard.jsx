...
import { api, fetchInventoryItems /* FORCE RELOAD v2 */ } from '../lib/api'
...
// Base popup component with weather AND inventory
function BasePopup({ base, baseWeather, loadBaseWeather, inventory }) {
  ...
  // Filter inventory for this base
  ...
  const lowStockItems = baseInventory.filter(item => {
    ...
    <div style={{ minWidth: 280 }}>
      ...
      <div style={{ marginBottom: 12, paddingBottom: 12, borderBottom: '1px solid #ddd' }}>
        ...
        {bw.loading ? (
          ...
          <div style={{ color: 'red', fontSize: 12 }}>
            Weather unavailable<br />
            <button onClick={() => loadBaseWeather(base)} style={{ marginTop: 6, padding: '4px 8px', fontSize: 11 }}>
              Retry
            </button>
          </div>
        ) : bw.data ? (
          <div style={{ fontSize: 12, marginTop: 4 }}>
            ...
            Humidity: {bw.data.humidity}% | Wind: {bw.data.wind_speed} m/s
          </div>
        ) : (
          <button onClick={() => loadBaseWeather(base)} style={{ padding: '4px 8px', fontSize: 11, marginTop: 4 }}>
            Load Weather
          </button>
        )}
      </div>
      ...
      <div>
        ...
        {baseInventory.length === 0 ? (
          <div style={{ fontSize: 12, color: '#999', marginTop: 4 }}>No inventory at this base</div>
        ) : (
          <div style={{ fontSize: 12, marginTop: 4 }}>
            <div style={{ marginBottom: 6 }}>
              <strong>{baseInventory.length}</strong> item types | <strong>{totalItems.toLocaleString()}</strong> total units
            ...
            <div style={{ maxHeight: 150, overflowY: 'auto', border: '1px solid #ddd', borderRadius: 4, padding: 6 }}>
              {baseInventory.map(item => {
                const percentage = (item.quantity / item.min_stock_level) * 100
                const color = percentage <= 50 ? '#ff4444' : percentage <= 100 ? '#ff9800' : '#4CAF50'
      ...
      <div style={{ marginTop: 16 }}>
        <div className='card'>
          <div style={{
            ...
            justifyContent: 'space-between',
            alignItems: 'center',
            padding: showInventory ? '0 0 16px 0' : 0,
            cursor: 'pointer'
          }}
          onClick={() => setShowInventory(!showInventory)}
          >
            <h4 style={{ margin: 0 }}>📦 Inventory Management ({inventory.length || 0} items)</h4>
            <button
              className='btn'
