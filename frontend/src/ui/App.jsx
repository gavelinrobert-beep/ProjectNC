...
export default function App(){
  ...
  return (
    <>
      ...
      >
        <span>PROJECT AEGIS — C2</span>
        <span style={{ fontWeight: 400 }}>
          Roll: <b>{role}</b>{' '}
          {role !== 'anonymous' && (
            <button
              className="btn"
              onClick={() => { logout(); setRole('anonymous') }}
              style={{ marginLeft: 8 }}
            >
              Logga ut
            </button>
        ...
        {tab === 'admin' ? (
          <div className="wrap">
            <div className="card">
              ...
              {role === 'anonymous'
                ? <div className="content"><Login onSuccess={(r)=>setRole(r)} /></div>
                : <AdminGeofences />}
            </div>
          </div>
        ) : (
            ...
            <div className="card" style={{ padding: '.75rem' }}>
              <MapContainer center={[62.3901, 17.3062]} zoom={11} id="map">
                ...
                {assets.map(a => (
                  <Marker
                    key={a.id}
                    position={[62.39 + Math.random() * 0.01, 17.31 + Math.random() * 0.01]}
                    icon={L.icon({
